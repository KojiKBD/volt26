...
- `VOLT26.Downloads`: dormant, transport-neutral download-job registry and immutable status snapshots for future event providers. It normalizes progress and terminal states for presentation but does not authorize network requests, archive extraction, retry, cancellation, or song reloads.
